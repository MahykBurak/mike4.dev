    ...
    <div className="flex h-16 items-center space-x-2">
      {!data.isPlaying ? (
        <span className="font-light">
          Şu anda çalmıyor -{' '}
          <span className="text-gray-500 dark:text-gray-400">Spotify</span>
        </span>
      ) : (
        <div className="flex items-center justify-center space-x-1">
          ...
          >
            ...
            <span className="font-light">{data.artistName}</span>
          </Link>
        </div>
      )}
      <Link href="/topsongs" rel="icon">
        <svg
          className={clsx(
            'ml-auto mt-[-2px] h-8 w-8',
